...
func (w *WorkoutS) SaveStudentsWorkout(c *gin.Context) {
	...
	// Kayıt, yapılan planın kendisine değil programın köküne bağlanır ki aynı
	// programın önceki seansıyla karşılaştırılabilsin.
	root := *totalWorkout.SourcePlanID
	if workoutDb.SourcePlanID != nil {
		root = *workoutDb.SourcePlanID
	}
...
func (w *WorkoutS) CopyWorkout(c *gin.Context) {
	...
	workoutCopied.ID = 0
	// Kopya, kökündeki programa bağlanır: aynı programın bütün seansları tek bir
	// source_plan_id altında toplanır, gelişim onlar arasında ölçülür.
	root := workoutId
	if workoutCopied.SourcePlanID != nil {
		root = *workoutCopied.SourcePlanID
	}
	workoutCopied.SourcePlanID = &root
